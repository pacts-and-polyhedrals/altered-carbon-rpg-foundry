...
test('latest manifest is stable but its archive is version-pinned', () => {
  ...
  assert.equal(current.manifest, 'https://github.com/example/project/releases/latest/download/system.json');
});
...
test('validator catches the old raw-branch manifest path',()=> {
  const manifest=makeManifest(); manifest.manifest='https://raw.githubusercontent.com/example/actual-project/main/system.json';
...
test('release workflow is explicit, uploads a standalone manifest, and verifies after publication',()=> {
  ...
  assert.match(workflow,/npm run prepare:release/); assert.match(workflow,/npm run release:check/);
  ...
  assert.ok(script.indexOf('gh release edit')<script.indexOf('node scripts/verify-published.mjs'));
