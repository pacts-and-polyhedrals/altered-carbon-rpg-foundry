...
Hooks.once('init',()=>{
  console.log('Altered Carbon RPG | Initializing v1.4.2');
  ...
  game.alteredCarbon={diagnoseSystem,BonusDice:{grant:grantBonusDice,get:getBonusDiceAwards,remove:removeBonusDiceAward,clear:clearBonusDiceAwards},Rules,Combat,Opposed,ChatActions,Book:bookAPI(),openAdvancement,CoreContent:{loadCoreCatalog,addCoreItemById,installCoreLibraryToWorld},version:'1.4.2',openCombatConsole:()=>new ACCombatConsole().render({force:true}),openCharacterCreator:()=>new ACCharacterCreator().render({force:true}),openRules:actor=>new ACRulesBrowser({actorId:actor?.id}).render({force:true}),openCoreLibrary:actor=>new ACCoreLibrary({actorId:actor?.id}).render({force:true}),openAdventureBook:()=>bookAPI().openBook(),importAdventureBook:options=>bookAPI().importBook(options),openGMControl:()=>new ACGMPanel().render({force:true}),openGMGuide:()=>ensureGMGuide({open:true})};
  registerBook();
