...
Hooks.once('init',()=>{
  console.log('Altered Carbon RPG | Initializing v1.0.0');
  ...
  game.alteredCarbon={Rules,Combat,Opposed,ChatActions,version:'1.0.0',openCombatConsole:()=>new ACCombatConsole().render({force:true}),openCharacterCreator:()=>new ACCharacterCreator().render({force:true}),openRules:actor=>new ACRulesBrowser({actorId:actor?.id}).render({force:true})};
