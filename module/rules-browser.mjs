...
import {ACCoreLibrary,loadCoreCatalog,addCatalogItemToActor} from './core-content.mjs';
...
export class ACRulesBrowser extends HandlebarsApplicationMixin(ApplicationV2){
 static DEFAULT_OPTIONS={id:'ac-rules-browser',classes:['altered-carbon','ac-rules-window'],window:{title:'Altered Carbon — 2020 Rules Reference'},position:{width:980,height:840},actions:{add:this._add,openCoreLibrary:this._openCoreLibrary}};
 ...
 async _prepareContext(options){
   ...
   const [skills,traits,baggage,mechanics,sleeveRef,equipmentRef,gmRef,coreCatalog]=await Promise.all([
     loadJSON('core-skills.json'),loadJSON('trait-catalog.json'),loadJSON('baggage-catalog.json'),loadJSON('mechanics-reference.json'),loadJSON('sleeve-reference-pages.json'),loadJSON('equipment-reference-pages.json'),loadJSON('gm-reference-pages.json'),loadCoreCatalog()
   ]);
   const actor=game.actors.get(this.actorId)||null;
   const coreGroups=coreCatalog.groups.map(group=>({...group,items:group.items.map(entry=>({...entry,canAdd:Boolean(actor)}))}));
   return {...context,actor,isGM:game.user.isGM,skills,coreItemCount:coreCatalog.items.length,coreGroups,coreVehicleCount:coreCatalog.vehicles.length,traits:traits.traits,baggage:baggage.entries,mechanics,conditions:mechanics.conditions,injuries:mechanics.injuries,scandals:mechanics.scandals,gearRules:mechanics.gearRules,sleeveReference:sleeveRef.pages,equipmentReference:equipmentRef.pages,gmReference:gmRef.pages};
 }
 static async _add(event,target){
   ...
   if(type==='core-item'){const catalog=await loadCoreCatalog();const entry=catalog.items.find(x=>x.id===id);if(entry){await addCatalogItemToActor(actor,entry);return;}}
   if(data){
 ...
 static async _openCoreLibrary(){new ACCoreLibrary({actorId:this.actorId}).render({force:true});}
