...
import {resolveBonusDice,describeBonusDice} from './gm-roll-options.mjs';
...
const activeSkillRolls=new Set();
export async function rollSkill(actor, skill, options={}){
  if(!game.user.isGM&&!actor.isOwner)throw new Error('You do not own this character.');
  const key=actor.uuid||actor.id;
  if(activeSkillRolls.has(key)){const reason='A Skill Check is already in progress for this character.';ui.notifications.warn(reason);return {blocked:true,reason};}
  activeSkillRolls.add(key);
  try{return await performSkillRoll(actor,skill,options);}finally{activeSkillRolls.delete(key);}
}
async function performSkillRoll(actor, skill, options={}){
  ...
  const awards=matchingBonusDiceAwards(actor,skill);
  const extraDice=[...resolveBonusDice(options.bonusDice||[],sides),...awardedDiceForRoll(awards,sides)];
  for(const award of awards)sit.notes.push(`GM: ${award.label} (${describeBonusDice(award.dice)}; ${award.durationLabel})`);
  const skillRoll=await new Roll(`1d${sides}`).evaluate(), original=dieResult(skillRoll);
  const bonusDice=[];for(const d of extraDice){const ds=Number(d);if(!ds)continue;const r=await new Roll(`1d${ds}`).evaluate();bonusDice.push({sides:ds,result:dieResult(r),roll:r});}
